...
public class SillyPuttyType extends FunkyFluidType {
	...
	public SillyPuttyType(Properties properties, FunkyFluidInfo info) {
	...
	public boolean move(FluidState state, LivingEntity entity, Vec3 movementVector, double gravity) {
		...
		if (depth > 0.0d && depth < 0.2d && Math.abs(entity.getDeltaMovement().length()) > MIN_SPEED / 2.0D) { //if entity is moving fast enough and is at the surface of fluid, behave as a solid
			...
			vec32 = vec32.scale(BASE_RESISTANCE / (1.0D + Math.abs(entity.getDeltaMovement().length() * RESISTANCE_MULTIPLIER))); //add more friction depending on speed of entity
	...
	public void setItemMovement(ItemEntity entity) {
		...
		entity.setDeltaMovement(motion.scale(BASE_RESISTANCE / (1.0D + Math.abs(motion.length() * RESISTANCE_MULTIPLIER)))); //add more friction depending on speed of entity
	}
}
